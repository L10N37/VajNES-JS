      ...
      // loop through all td elements that do not contain the class 'addressClass'
      document.querySelectorAll('#workRamTable td:not(.addressClass)').forEach(function(td, i) {
        // assign the appropriate id to the element
        let id = 'ramByte' + i;
        td.setAttribute('id', id);
        // add the id to the ramByte array for later use
        ramByte.push(id);
        // add click event listener to the td element
        td.addEventListener('click', function(event) {
          let index = ramByte.indexOf(id);
          document.querySelector('locContainer').innerHTML='&nbsp'+hexPrefix+index.toString(16);
        });
      });
